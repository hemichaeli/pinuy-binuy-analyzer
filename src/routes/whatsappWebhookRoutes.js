 ...
 * v2.1 - Fixed service dependencies and error handling
 */
...
// Trello Integration Service (Safe)
class TrelloService {
  static async createCardForHotLead(leadId, phone, intelligence) {
    try {
      ...
      if (!process.env.TRELLO_API_KEY || !process.env.TRELLO_TOKEN || !process.env.TRELLO_BOARD_ID) {
        logger.info('🎯 Trello not configured, skipping card creation');
        return;
      }
      ...
      // Try to use existing trello service if available
      try {
        ...
        logger.warn('🎯 Trello service not available', { error: serviceError.message });
      }
...
router.post('/whatsapp/webhook', async (req, res) => {
  try {
    ...
    // Step 5: Create Trello card for hot leads (safe)
...
router.post('/whatsapp/trigger', async (req, res) => {
  try {
    ...
    logger.info('🔧 Enhanced trigger', { phone, message: message.substring(0, 50) });
