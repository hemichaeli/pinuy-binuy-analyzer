...
// GET /api/opportunities - Top investment opportunities (with enriched data)
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        c.developer_status, c.developer_risk_level, c.developer_risk_score,
    ...
    res.json({
      ...
      opportunities: result.rows.map(row => ({
        ...row,
        price_per_sqm: row.accurate_price_sqm,
...
router.get('/listings/filter-options', async (req, res) => {
  try {
    ...
    const [cities, complexes, priceRange, roomsRange, areaRange, floorRange, dealStatuses, messageStatuses] = await Promise.all([
      pool.query(`SELECT city, COUNT(*) as count FROM listings WHERE is_active = TRUE AND city IS NOT NULL GROUP BY city ORDER BY count DESC`),
      pool.query(`SELECT c.id, c.name, c.city, c.status, c.iai_score, COUNT(l.id) as listing_count FROM complexes c JOIN listings l ON l.complex_id = c.id AND l.is_active = TRUE${cityCondition} GROUP BY c.id HAVING COUNT(l.id) > 0 ORDER BY c.city, c.name`),
      pool.query(`SELECT MIN(asking_price) as min_price, MAX(asking_price) as max_price, AVG(asking_price) as avg_price FROM listings WHERE is_active = TRUE AND asking_price > 0`),
      pool.query(`SELECT MIN(rooms) as min_rooms, MAX(rooms) as max_rooms FROM listings WHERE is_active = TRUE AND rooms > 0`),
      pool.query(`SELECT MIN(area_sqm) as min_area, MAX(area_sqm) as max_area FROM listings WHERE is_active = TRUE AND area_sqm > 0`),
      pool.query(`SELECT MIN(floor) as min_floor, MAX(floor) as max_floor FROM listings WHERE is_active = TRUE AND floor IS NOT NULL`),
      pool.query(`SELECT deal_status, COUNT(*) as count FROM listings WHERE is_active = TRUE GROUP BY deal_status ORDER BY count DESC`),
      pool.query(`SELECT message_status, COUNT(*) as count FROM listings WHERE is_active = TRUE GROUP BY message_status ORDER BY count DESC`)
    ]);

    res.json({
      cities: cities.rows, complexes: complexes.rows,
      price_range: priceRange.rows[0], rooms_range: roomsRange.rows[0],
      area_range: areaRange.rows[0], floor_range: floorRange.rows[0],
      deal_statuses: dealStatuses.rows, message_statuses: messageStatuses.rows
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/listings/search', async (req, res) => {
  try {
    const {
      city, complex_id, min_price, max_price, min_rooms, max_rooms,
      min_area, max_area, min_floor, max_floor, min_ssi, max_ssi, min_iai,
    ...
    const addFilter = (col, op, val, parse = parseFloat) => {
      if (val !== undefined && val !== null && val !== '') {
        paramCount++;
        conditions.push(`${col} ${op} $${paramCount}`);
        params.push(parse(val));
      }
    };

    addFilter('l.city', '=', city, v => v);
    addFilter('l.complex_id', '=', complex_id, parseInt);
    addFilter('l.asking_price', '>=', min_price);
    addFilter('l.asking_price', '<=', max_price);
    addFilter('l.rooms', '>=', min_rooms);
    addFilter('l.rooms', '<=', max_rooms);
    addFilter('l.area_sqm', '>=', min_area);
    addFilter('l.area_sqm', '<=', max_area);
    addFilter('l.floor', '>=', min_floor, parseInt);
    addFilter('l.floor', '<=', max_floor, parseInt);
    addFilter('l.ssi_score', '>=', min_ssi, parseInt);
    addFilter('l.ssi_score', '<=', max_ssi, parseInt);
    addFilter('c.iai_score', '>=', min_iai, parseInt);
    addFilter('l.days_on_market', '>=', min_days_on_market, parseInt);
    addFilter('l.days_on_market', '<=', max_days_on_market, parseInt);
    addFilter('l.price_changes', '>=', min_price_drops, parseInt);
    if (has_price_drop === 'true') conditions.push(`l.total_price_drop_percent > 0`);
    if (is_foreclosure === 'true') conditions.push(`l.is_foreclosure = TRUE`);
    if (is_inheritance === 'true') conditions.push(`l.is_inheritance = TRUE`);
    if (has_urgent_keywords === 'true') conditions.push(`l.has_urgent_keywords = TRUE`);
    ...
    const [result, countResult] = await Promise.all([
      pool.query(`
        SELECT l.id as listing_id, l.source, l.url, l.source_listing_id,
          l.asking_price, l.area_sqm, l.rooms, l.floor, l.price_per_sqm,
          l.days_on_market, l.price_changes, l.total_price_drop_percent, l.original_price,
          l.has_urgent_keywords, l.urgent_keywords_found, l.is_foreclosure, l.is_inheritance,
          l.ssi_score, l.ssi_time_score, l.ssi_price_score, l.ssi_indicator_score,
      ...
      pool.query(`SELECT COUNT(*) as total FROM listings l JOIN complexes c ON l.complex_id = c.id WHERE ${whereClause}`, params)
    ]);
