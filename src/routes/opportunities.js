...
router.get('/listings/search', async (req, res) => {
  try {
    const {
      ...
      min_ssi, max_ssi, min_iai,
      min_days_on_market, max_days_on_market,
      min_price_drops, has_price_drop,
    ...
    const countQuery = `SELECT COUNT(*) as total FROM listings l JOIN complexes c ON l.complex_id = c.id WHERE ${whereClause}`;

    const dataQuery = `
      SELECT 
        l.id as listing_id, l.source, l.url, l.source_listing_id,
