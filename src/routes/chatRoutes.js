/**
 * QUANTUM Chat - Ask Perplexity questions about our DB
 * POST /api/chat/ask - Ask a question, get AI answer based on DB data
 * GET /api/chat/ - Simple chat UI
 */
...
IAI = מדד אטרקטיביות השקעה (0-100, פער פרמיה + שלב תכנוני + יזם). SSI = מדד לחץ מוכר (0-100, כינוס/הוצלפ/ירושה/הורדות מחיר).

`;

  // Detect city in question
  const allCities = (await pool.query(`SELECT DISTINCT city FROM complexes WHERE city IS NOT NULL`)).rows;
  let targetCity = null;
  for (const c of allCities) {
    if (c.city && lowerQ.includes(c.city.toLowerCase())) {
      targetCity = c.city;
      break;
    }
  }
  ...
  if (lowerQ.includes('מצוק') || lowerQ.includes('לח') || lowerQ.includes('stress') || lowerQ.includes('ssi') || lowerQ.includes('כינוס') || lowerQ.includes('ירוש') || lowerQ.includes('מוכר') || lowerQ.includes('הזדמנו') || lowerQ.includes('זול') || lowerQ.includes('מתחת')) {
    const stressed = (await pool.query(`
      SELECT name, city, enhanced_ssi_score, iai_score, is_receivership, 
        is_inheritance_property, has_enforcement_cases, ssi_enhancement_factors
      FROM complexes WHERE enhanced_ssi_score >= 10 ORDER BY enhanced_ssi_score DESC LIMIT 15
    `)).rows;
    
    context += `=== מוכרים במצוקה (SSI 10+) ===\n`;
  ...
  if (lowerQ.includes('כינוס') || lowerQ.includes('kones') || lowerQ.includes('receivership') || lowerQ.includes('מכר')) {
    ...
    if (kones.length > 0) {
      context += `=== כינוס נכסים (${kones.length} פעילים) ===\n`;
      kones.forEach((k, i) => {
        context += `${i+1}. ${k.address || '-'} | ${k.city || '-'} | ${k.property_type || '-'} | ${k.price ? '₪'+Number(k.price).toLocaleString() : '-'} | מועד: ${k.submission_deadline || '-'}\n`;
      });
      context += '\n';
    }
  }
  ...
  if (lowerQ.includes('עסק') || lowerQ.includes('מכיר') || lowerQ.includes('מחיר') || lowerQ.includes('transaction') || lowerQ.includes('נמכר')) {
    ...
    if (tx.length > 0) {
      ...
      tx.forEach((t, i) => {
        context += `${i+1}. ${t.address || '-'} | ${t.city || '-'} | ₪${t.price ? Number(t.price).toLocaleString() : '-'} | ${t.size_sqm || '-'}מר | ₪${t.price_per_sqm ? Number(t.price_per_sqm).toLocaleString() : '-'}/מר | ${t.transaction_date ? new Date(t.transaction_date).toLocaleDateString('he-IL') : '-'}\n`;
      });
      context += '\n';
    }
  }

  // City breakdown if comparing cities
  if (lowerQ.includes('עיר') || lowerQ.includes('ערים') || lowerQ.includes('השוו') || lowerQ.includes('איפה') || lowerQ.includes('איזה עיר') || lowerQ.includes('הכי טוב')) {
    const cities = (await pool.query(`
      SELECT city, COUNT(*) as total, 
        COUNT(*) FILTER (WHERE iai_score >= 30) as opp,
        COUNT(*) FILTER (WHERE enhanced_ssi_score >= 40) as stressed,
        ROUND(AVG(iai_score)) as avg_iai
    ...
    context += `=== פילוח ערים ===\n`;
    cities.forEach(c => {
      context += `${c.city}: ${c.total} מתחמים, ${c.opp} הזדמנויות, ${c.stressed} לחוצים, IAI ממוצע ${c.avg_iai}\n`;
    });
    context += '\n';
  }

  // Golden opportunities (high IAI + high SSI)
  ...
  if (golden.length > 0) {
    context += `=== הזדמנויות זהב (IAI גבוה + SSI גבוה) ===\n`;
    golden.forEach((g, i) => {
      context += `${i+1}. ${g.name} | ${g.city} | IAI:${g.iai_score} | SSI:${g.enhanced_ssi_score} | ${g.developer || '-'} | ${g.status || '-'}\n`;
    });
  }
...
const SYSTEM_PROMPT = `אתה יועץ השקעות נדל"ן מומחה של QUANTUM - משרד תיווך מוביל בתחום פינוי-בינוי בישראל.
יש לך גישה למסד נתונים עם מידע על מאות מתחמי פינוי-בינוי בכל הארץ.

כללים:
- ענה בעברית תמיד
- תבסס תשובות על הנתונים שקיבלת - אל תמציא מספרים
- תן תובנות חכמות, לא רק פירוט יבש
- הסבר למה הזדמנות טובה או לא
- אם נשאלת על משהו שאין לך מידע עליו, אמור זאת בכנות
- השתמש במונחים: IAI (מדד אטרקטיביות השקעה), SSI (מדד לחץ מוכר)
- תהיה ישיר, חד, ומועיל
- אל תציין שקיבלת נתונים ממסד נתונים - פשוט דע את המידע`;

/**
 * POST /api/chat/ask - Ask a question about the DB
 */
router.post('/ask', async (req, res) => {
  try {
    ...
    if (!apiKey) return res.status(500).json({ error: 'Perplexity API not configured' });

    logger.info(`Chat question: ${question.substring(0, 100)}`);
    ...
    logger.info(`Chat answer: ${answer.substring(0, 100)}...`);
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header .badge { background: #10b981; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 10px; }
...
<body>
  <div class="header">
    <h1>QUANTUM Chat</h1>
    <span class="badge">AI + DB</span>
  </div>
  
  <div class="chat-area" id="chat">
    <div class="msg system">שלום! אני יודע הכל על מתחמי פינוי-בינוי בישראל. שאל אותי כל שאלה.</div>
  </div>
  ...
  <script>
    ...
    async function send() {
      ...
      addMsg(q, 'user');
      const typing = addMsg('חושב...', 'typing');
      ...
      try {
        ...
        if (data.error) {
          addMsg('שגיאה: ' + data.error, 'system');
        } else {
        ...
        typing.remove();
        addMsg('שגיאת חיבור: ' + e.message, 'system');
      }
