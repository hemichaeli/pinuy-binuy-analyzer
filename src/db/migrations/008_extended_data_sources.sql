-- Migration 008: Extended Data Sources - Phase 4.5
-- SSI Enhancement, Pricing Accuracy, News Monitoring
...
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS accurate_price_sqm NUMERIC;
...
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS estimated_premium_price NUMERIC;
...
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS city_avg_price_sqm NUMERIC;
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS price_vs_city_avg NUMERIC;
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS cbs_price_index NUMERIC;
ALTER TABLE complexes ADD COLUMN IF NOT EXISTS yearly_price_change NUMERIC;
...
CREATE INDEX IF NOT EXISTS idx_complexes_enhanced_ssi ON complexes(enhanced_ssi_score DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_complexes_accurate_price ON complexes(accurate_price_sqm);
CREATE INDEX IF NOT EXISTS idx_complexes_price_confidence ON complexes(price_confidence_score);
CREATE INDEX IF NOT EXISTS idx_complexes_news_sentiment ON complexes(news_sentiment);
...
CREATE INDEX IF NOT EXISTS idx_distressed_sellers_score ON distressed_sellers(distress_score DESC);
...
COMMENT ON COLUMN complexes.enhanced_ssi_score IS 'Enhanced SSI score including distress indicators';
COMMENT ON COLUMN complexes.ssi_enhancement_factors IS 'JSON array of factors contributing to SSI enhancement';
COMMENT ON COLUMN complexes.accurate_price_sqm IS 'Multi-source weighted average price per sqm';
COMMENT ON COLUMN complexes.price_confidence_score IS 'Confidence score (0-100) based on data availability';
COMMENT ON COLUMN complexes.estimated_premium_price IS 'Estimated post-renewal price per sqm';
COMMENT ON COLUMN complexes.news_sentiment IS 'Overall news sentiment: positive, negative, neutral, mixed';
COMMENT ON COLUMN complexes.developer_reputation_score IS 'Developer reputation score 1-10';

COMMENT ON TABLE news_alerts IS 'News and regulatory alerts for complexes';
COMMENT ON TABLE distressed_sellers IS 'Identified distressed sellers for targeted outreach';
COMMENT ON TABLE price_history IS 'Historical price data for trend analysis';
COMMENT ON TABLE regulation_updates IS 'Pinuy Binuy related regulation updates';
