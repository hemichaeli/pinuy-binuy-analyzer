...
const PERPLEXITY_MODEL = 'sonar'; // Must match perplexityService.js
...
async function fetchFromNadlanGov(street, city, houseNum = '') {
  try {
    ...
    // Detect HTML responses (API returns web page instead of JSON)
...
אם אין מידע, החזר: {"transactions":[],"neighborhood_avg_price_sqm":null}`;
    ...
    try {
      ...
      logger.warn(`Failed to parse Perplexity response for ${complexName}: ${e.message}`);
      return { transactions: [], neighborhoodAvg: null };
    }
    ...
    try {
      ...
      if (tx.source_id) {
        const existing = await pool.query(
          'SELECT id FROM transactions WHERE complex_id = $1 AND source = $2 AND source_id = $3',
      ...
      if (tx.transaction_date && tx.price) {
        const dupCheck = await pool.query(
          'SELECT id FROM transactions WHERE complex_id = $1 AND transaction_date = $2 AND price = $3 AND address = $4',
 ...
 * Tries nadlan.gov.il first, always falls back to Perplexity
 */
async function scanComplex(complexId) {
  try {
    ...
    if (addressList.length > 0) {
      for (const addr of addressList.slice(0, 5)) {
    ...
    // ALWAYS try Perplexity if no transactions found
    ...
    // Calculate from transactions if no Perplexity avg
    ...
    return {
      ...
      newTransactions, source, neighborhoodAvg, apiAvailable
    };
...
async function scanAll(options = {}) {
  ...
  const results = {
    ...
    scanned: 0, succeeded: 0, failed: 0, totalNew: 0, withNeighborhoodAvg: 0,
    details: []
  };
