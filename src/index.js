...
app.post('/api/morning-report/send', async (req, res) => { try { const { sendMorningReport } = require('./services/morningReportService'); res.json({ message: 'Morning report generating...', note: 'Report will be sent to configured emails' }); const result = await sendMorningReport(); } catch (e) { res.status(500).json({ error: e.message }); } });
app.get('/api/morning-report/preview', async (req, res) => { try { const { sendMorningReport } = require('./services/morningReportService'); const result = await sendMorningReport(); res.json(result); } catch (e) { res.status(500).json({ error: e.message }); } });
...
async function start() {
  ...
  // Morning Intelligence Report - Daily 07:30 Israel Time
  try {
    const cron = require('node-cron');
    cron.schedule('30 7 * * *', async () => {
      logger.info('[MorningReport] Cron triggered at 07:30');
      try {
        const { sendMorningReport } = require('./services/morningReportService');
        const result = await sendMorningReport();
        logger.info('[MorningReport] Completed:', JSON.stringify(result.stats || {}));
      } catch (e) {
        logger.error('[MorningReport] Cron failed:', e.message);
      }
    }, { timezone: 'Asia/Jerusalem' });
    logger.info('[MorningReport] Scheduled daily at 07:30 Asia/Jerusalem');
  } catch (e) {
    logger.warn('[MorningReport] Failed to schedule:', e.message);
  }
